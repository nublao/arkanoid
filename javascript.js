...
function Ladrillo(id, posXladrillo, posYladrillo) {
  ...
  this.colorLadrillo = 'rgb(180, 75, 75)';
  this.golpeado = false;
  this.golpe = false;
  this.crearLadrillo();
}
...
function ganarPuntos(id) {
  ...
  if(contDestruidos == cantLadrillos) {
    hasGanado();
  }
}
...
function desactivarObjetos() {
  clearInterval(pelotaInicial.timerPelota);
...
function hasGanado() {
  ...
  txtJugar.style.visibility = 'visible';
...
window.onload = function() {
  ...
  ladrillos[0] = new Ladrillo(0, 50, 50);
  ...
  // Escribimos los textos de información
  ...
  txtJugar = document.getElementById('textoJugarOtraVez');
